...
Aplicación Angular para operar RedDiff desde el navegador. Incluye inicio y cierre de sesión, restauración segura de la sesión, administración de usuarios, inventario de dispositivos, gestión protegida del acceso remoto, captura SSH de solo lectura, evidencia histórica, comparación diferencial y verificación de cumplimiento contra líneas base según el rol autenticado.
...
La compilación de producción mantiene carga diferida para las páginas de acceso, inicio, usuarios, dispositivos, capturas, comparaciones y cumplimiento. La gestión del acceso remoto permanece dentro del módulo de dispositivos y solo se presenta al administrador. La página de capturas muestra la operación SSH únicamente para equipos autorizados con acceso completo. La página de comparaciones exige dos versiones del mismo dispositivo y presenta evidencia de solo lectura. La página de cumplimiento permite a ambos roles verificar una versión compatible, pero reserva al administrador la creación y el cambio de estado de las líneas base. El servidor sigue siendo la autoridad final para validar cada solicitud.
